...
void AnalysisHandler::isolateElectrons() {
    ...
    for (int e = 0; e < electrons.size(); e++) {
        ...
        for (int i = 0; i < listOfElectronTags.size(); i++) {
            ...
            // cout << "Electron isolation for:" << iso->source << " , electronPT = " << cand->PT << ", sumPT = " << 10000.0*sumPT << ", maxVal = " << iso->maxVal << endl;
            if (sumPT < iso->maxVal)
        ...
        for (int i = 0; i < listOfMuonTags.size(); i++) {
            ...
            if (iso->source == "t") {
                for (int t = 0; t < tracks.size(); t++) {
                    ...
                    if (neighbour->P4().DeltaR(cand->P4()) > maxDR)
                        continue;
                    // Ignore the muon's track itself
                     if(neighbour->Particle == cand->Particle)
                         continue;
            ...
            if (iso->divideByPTCand)
                sumPT /= cand->PT;
            // now check against limit and if so, add the correct tag value
            // cout << "Muon isolation for:" << iso->source << " , muonPT = " << cand->PT << ", sumPT = " << 10000.0*sumPT << ", maxVal = " << iso->maxVal << endl;
            if (sumPT < iso->maxVal)
                flags.push_back(true);
            else
                flags.push_back(false);
