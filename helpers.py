...
default_slha_dir = Path("./slhaFiles").resolve()

SR_LINE_RE = re.compile(r"^(SR-[A-Z]{2}-[01]J[a-i])\s+\S+\s+\S+\s+(\S)+\s+(\S+)")

def get_model_pars(slha_file: Path, parent_PDG = 5000011, 
                    daughter_PDG = 5000022) -> Dict[str, float]:

    slha = pyslha.readSLHAFile(str(slha_file))
    pars = namedtuple('ModelPars', ['mParent','mDaughter'])

    mParent = slha.blocks['MASS'].get(parent_PDG, -1.0)
    mDaughter = slha.blocks['MASS'].get(daughter_PDG, -1.0)

    return pars(mParent, mDaughter)


def to_cut_name(sr: str) -> str:
    channel, jetbin = sr.removeprefix("SR-").split("-")
    return f"SR{channel}_{jetbin[0]}{jetbin[2]}_cuts"


def get_signal_yieldsFrom(path: Path) -> Dict[str, float]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if m:
            sr_to_acc[to_cut_name(m.group(1))] = float(m.group(3))
...
def get_poi_upper_limit(signal_yields: Dict[str, float], 
                         bg_json_file : Path = Path('./bkgonly.json'),
